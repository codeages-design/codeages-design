...
class Layout extends Component {
  ...
  buildTree(){
    ...
    for(let item of data){
        this.options.output = ''
        html += `<li class="cd-group-item cd-group-item_outer"><a id=${item.id}  class='cd-group-item_link' title=${item.name} target='${item.linkType ? '_blank': '_self'}' data-grade=${item.grade} href=${item.link ? item.link : 'javascript:;'}  data-href=${item.link} ><span class='item-txt'>${item.name}</span></a>`;
  ...
  creatElement(item){
    if(item.nodes && item.nodes.length){
        ...
        for(let list of item.nodes){
            this.options.output += `<li class='cd-group-item'><a id=${list.id} class='cd-group-item_link' title=${list.name} target='${list.linkType ? '_blank': '_self'}' data-grade=${list.grade} href=${list.link ? list.link : 'javascript:;'} data-href=${list.link}><span class='item-txt'>${list.name}</span></a>`;
